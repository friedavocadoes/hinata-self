...
using (id = auth.uid() or public.is_finance_admin());
...
-- Product policy deliberately allows only non-sensitive columns through views/API in the app.
-- Finance admins can manage the full table.
...
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage incoterms" on public.incoterms for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage payment terms" on public.payment_terms for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage vehicle types" on public.vehicle_types for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage transport" on public.transport_rates for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage transport surcharges" on public.transport_surcharges for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage settings" on public.global_settings for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage bank rules" on public.bank_fee_rules for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage insurance rules" on public.insurance_rules for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());
create policy "admins manage customs rules" on public.customs_charge_rules for all to authenticated
using (public.is_finance_admin()) with check (public.is_finance_admin());

-- Quotations: reps can work with their own quotes; admins see everything.
create policy "read quotations" on public.quotations for select to authenticated
using (created_by = auth.uid() or public.is_finance_admin());
create policy "create quotations" on public.quotations for insert to authenticated
with check (created_by = auth.uid());
create policy "update quotations" on public.quotations for update to authenticated
using (created_by = auth.uid() or public.is_finance_admin())
with check (created_by = auth.uid() or public.is_finance_admin());
...
using (
  exists (
    ...
    where q.id = quotation_id
      and (q.created_by = auth.uid() or public.is_finance_admin())
  )
);
create policy "write quotation items" on public.quotation_items for all to authenticated
using (
  exists (
    select 1 from public.quotations q
    where q.id = quotation_id
...
-- Orders / purchases / inventory / tasks
...
using (exists (select 1 from public.orders o where o.id = order_id and (o.created_by = auth.uid() or public.is_finance_admin())));
create policy "write order items" on public.order_items for all to authenticated
using (exists (select 1 from public.orders o where o.id = order_id and (o.created_by = auth.uid() or public.is_finance_admin())))
with check (exists (select 1 from public.orders o where o.id = order_id and (o.created_by = auth.uid() or public.is_finance_admin())));

create policy "read purchases" on public.purchase_orders for select to authenticated
using (created_by = auth.uid() or public.is_finance_admin());
create policy "write purchases" on public.purchase_orders for all to authenticated
using (created_by = auth.uid() or public.is_finance_admin())
with check (created_by = auth.uid() or public.is_finance_admin());

create policy "read purchase items" on public.purchase_items for select to authenticated
using (exists (select 1 from public.purchase_orders p where p.id = purchase_order_id and (p.created_by = auth.uid() or public.is_finance_admin())));
create policy "write purchase items" on public.purchase_items for all to authenticated
using (exists (select 1 from public.purchase_orders p where p.id = purchase_order_id and (p.created_by = auth.uid() or public.is_finance_admin())))
with check (exists (select 1 from public.purchase_orders p where p.id = purchase_order_id and (p.created_by = auth.uid() or public.is_finance_admin())));
...
create policy "write inventory movements" on public.inventory_movements for insert to authenticated
with check (created_by = auth.uid() or public.is_finance_admin());

create policy "read tasks" on public.tasks for select to authenticated
using (assigned_to = auth.uid() or public.is_finance_admin());
create policy "write tasks" on public.tasks for all to authenticated
using (assigned_to = auth.uid() or public.is_finance_admin())
with check (assigned_to = auth.uid() or public.is_finance_admin());

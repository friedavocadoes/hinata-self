...
export async function createOrderFromQuotation(quotationId: string) {
  ...
  const { data: order, error: orderError } = await admin
    ...
    .insert({
      ...
      destination_name_snapshot: quotation.destination_name_snapshot,
      incoterm_code: quotation.incoterm_code,
      pay_term_code: quotation.pay_term_code,
      credit_period_days: quotation.credit_period_days,
      total_sales: quotation.total_sales,
  ...
  const { data: items, error: itemsError } = await admin
    ...
    .select("product_id, product_name_snapshot, qty_kg, sales_unit_price, sales_price, cost_per_unit, total_cost, profit_amount")
    .eq("quotation_id", quotationId);
  ...
  const { error: insertItemsError } = await admin.from("order_items").insert(
    (items ?? []).map((item) => ({
      ...
      qty_kg: item.qty_kg,
      unit_selling_price: item.sales_unit_price,
      total_sales: item.sales_price,
...
export async function updateOrderStatus(orderId: string, status: "draft" | "confirmed" | "partially_fulfilled" | "fulfilled" | "cancelled") {
  const { profile } = await requireUser();
  const admin = createAdminClient();
  const { data: order } = await admin.from("orders").select("created_by").eq("id", orderId).single();

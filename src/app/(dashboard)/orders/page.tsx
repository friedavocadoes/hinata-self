  ...
  return (
    <div className="p-8">
      ...
      <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
        ...
        {(orders ?? []).length === 0 ? <div className="p-12 text-center text-sm text-zinc-500">No orders yet.</div> : <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-sm"><thead className="border-b bg-zinc-50"><tr><th className="px-5 py-3 text-left">Order</th><th className="px-5 py-3 text-left">Customer</th><th className="px-5 py-3 text-left">Date</th><th className="px-5 py-3 text-left">Destination</th><th className="px-5 py-3 text-right">Sales</th><th className="px-5 py-3 text-left">Status</th></tr></thead><tbody className="divide-y">{(orders ?? []).map(order => <tr key={order.id} className="hover:bg-zinc-50"><td className="px-5 py-4"><Link href={`/orders/${order.id}`} className="font-medium hover:underline">{order.order_number}</Link></td><td className="px-5 py-4">{order.customer_name_snapshot}</td><td className="px-5 py-4">{order.order_date}</td><td className="px-5 py-4">{order.destination_name_snapshot ?? "—"}</td><td className="px-5 py-4 text-right font-medium">{money(Number(order.total_sales))}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium capitalize ring-1 ring-inset ${statusClass(order.status)}`}>{order.status.replaceAll("_", " ")}</span></td></tr>)}</tbody></table></div>}
      </section>
    </div>
  );
}

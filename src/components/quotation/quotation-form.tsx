...
type CostingItem = Extract<PreviewResult, { success: true }>['items'][number];
...
function SuccessItem({ item }: { item: CostingItem }) {
  return <>{"productName" in item ? item.productName : "Product"}</>;
}
...
export function QuotationForm({ customers, destinations, incoterms, paymentTerms, products }: Props) {
  ...
  async function handleCustomerChange(customerId: string) {
    ...
    try { setCustomerProducts(await getCustomerProducts(customerId)); } catch { setCustomerProducts([]); }
  }
  ...
  useEffect(() => {
    ...
    const creditDays = watchedValues.creditDays;
    const items = watchedItems;
    const totalQuantity = items.reduce((sum, item) => sum + Number(item.quantityKg || 0), 0);
    const vehicleType = getVehicleIdForQuantity(totalQuantity, vehicleTypes);

    if (!customerId || !destinationId || !incoterm || !paymentTerm || !Number.isFinite(creditDays) || !vehicleType || !items.some((item) => item.productId && Number(item.quantityKg) > 0)) {
      setPreviewResult(null);
      return;
    }
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const preview = await previewQuotation({
          customerId,
          deliveryType: watchedValues.deliveryType ?? "local",
          destinationId,
          incoterm,
          paymentTerm,
          creditDays,
          vehicleType,
          items: items.filter((item) => item.productId && Number(item.quantityKg) > 0).map((item, originalIndex) => {
            const index = items.findIndex((candidate) => candidate === item);
            const override = overrides[index] ?? overrides[originalIndex];
            return {
  ...
  }, [JSON.stringify(watchedValues), vehicleTypes, overrides]);
  ...
  function clearItemOverride(index: number) {
    setOverrides((current) => { const next = { ...current }; delete next[index]; return next; });
  }
  ...
  const liveItems = previewResult?.success ? previewResult.items : [];
  const selectedIncoterm = incoterms.find((item) => item.id === watchedValues.incoterm);
  const isExWorks = selectedIncoterm?.name?.toLowerCase().includes("ex works") || selectedIncoterm?.name?.toLowerCase().includes("ex-works");
  ...
  return (
    <div className="space-y-8">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        <section className="rounded-xl border bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            <div><label className="mb-2 block text-sm font-medium">Customer</label><SearchableSelect options={customers} value={watch("customerId")} onChange={handleCustomerChange} placeholder="Select customer" searchPlaceholder="Search customers..." /><FieldError message={errors.customerId?.message} /></div>
        ...
        {(previewResult?.success || previewResult?.success === false) && (
          <section className="rounded-xl border bg-white shadow-sm">
      ...
      {result?.success && <section className="rounded-xl border bg-white shadow-sm"><div className="flex items-center justify-between border-b p-6"><div className="flex items-center gap-3"><div className="rounded-full bg-emerald-50 p-2 text-emerald-600"><CheckCircle2 size={20} /></div><div><h2 className="text-base font-semibold">Quotation Saved</h2><p className="text-sm text-zinc-500">{result.quoteNumber}</p></div></div><p className="text-2xl font-semibold">{money(result.totalSales)}</p></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="border-b bg-zinc-50"><tr><th className="px-6 py-3 text-left">Product</th><th className="px-6 py-3 text-right">Qty</th><th className="px-6 py-3 text-right">Unit Selling Price</th><th className="px-6 py-3 text-right">Total</th></tr></thead><tbody className="divide-y">{result.items.map((item) => <tr key={item.productId}><td className="px-6 py-4 font-medium">{"productName" in item ? item.productName : "Product"}</td><td className="px-6 py-4 text-right">{number(item.quantityKg)} kg</td><td className="px-6 py-4 text-right">{money(item.salesUnitPrice)}</td><td className="px-6 py-4 text-right font-medium">{money(item.salesPrice)}</td></tr>)}</tbody></table></div><div className="flex justify-end border-t bg-zinc-50 px-6 py-4"><button type="button" onClick={() => setResult(null)} className="rounded-lg border bg-white px-3 py-2 text-sm font-medium hover:bg-zinc-50">Continue</button></div></section>}
    </div>
  );
}

...
type CostingItem = Extract<
  PreviewResult,
  { success: true; role: "finance_admin" }
>["items"][number];
type CostOverrides = {
...
function number(value: number) {
  return new Intl.NumberFormat("en-AE", { maximumFractionDigits: 2 }).format(
    value,
  );
}
...
export function QuotationForm({
  ...
  useEffect(() => {
    ...
    if (
      ...
      !vehicleType ||
      !items.some((item) => item.productId && Number(item.quantityKg) > 0)
    ) {
      setPreviewResult(null);
      return;
    }
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const preview = await previewQuotation({
          ...
          vehicleType,
          items: items
            .filter((item) => item.productId && Number(item.quantityKg) > 0)
  ...
  function updateOverride(
    index: number,
    key: keyof CostOverrides,
    value: string,
  ) {
    const numeric = value === "" ? undefined : Number(value);
    setOverrides((current) => ({
      ...current,
      [index]: { ...current[index], [key]: numeric },
    }));
  }
  ...
  const liveItems: CostingItem[] =
    previewResult?.success && previewResult.role === "finance_admin"
      ? previewResult.items
      : [];
  const selectedIncoterm = incoterms.find(
    (item) => item.id === watchedValues.incoterm,
  );
  const isExWorks =
    selectedIncoterm?.name?.toLowerCase().includes("ex works") ||
    selectedIncoterm?.name?.toLowerCase().includes("ex-works");
  const liveTotalCost = liveItems.reduce(
    (sum, item) => sum + item.totalCost,
    0,
  );
  const liveTotalSales = liveItems.reduce(
    (sum, item) => sum + item.salesPrice,
    0,
  );
  const liveTotalProfit = liveItems.reduce(
    (sum, item) => sum + item.profitAmount,
    0,
  );
  const liveMargin =
    liveTotalSales === 0 ? 0 : (liveTotalProfit / liveTotalSales) * 100;
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border bg-white px-3 py-2.5 pr-14 text-sm outline-none"
                />
        ...
        <section className="rounded-xl border bg-white shadow-sm">
          <div className="flex items-center justify-between border-b p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-500">
                Set quantity and margin. Costing below updates automatically
                from current stock and rules.
              </p>
            </div>
            <button
              type="button"
            ...
            <table className="w-full min-w-[760px] text-sm">
              <thead className="border-b bg-zinc-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left font-medium">Margin %</th>
                    ...
                    <td className="px-6 py-4">
                      <div className="relative max-w-[150px]">
                        <input
                          ...
                          className="w-full rounded-lg border px-3 py-2.5 pr-8 outline-none"
                        />
        ...
        {(previewResult?.success || previewResult?.success === false) && (
          <section className="rounded-xl border bg-white shadow-sm">
            <div className="flex items-center justify-between border-b p-6">
              <div>
                <h2 className="text-base font-semibold">Live Costing</h2>
                <p className="mt-1 text-sm text-zinc-500">
                  Unit cost and selling price update as you change margin or any
                  editable cost.
                </p>
              </div>
              {previewLoading && (
            ...
            ) : previewResult.role !== "finance_admin" ? (
              <div className="p-5 text-sm text-zinc-500">
                Live internal costing is available to Finance / Admin users.
              </div>
            ) : (
              <div className="overflow-visible">
                <table className="w-full min-w-[1250px] text-sm">
                  <thead className="border-b bg-zinc-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right">Warehouse Days</th>
                  ...
                  <tbody className="divide-y">
                    {liveItems.map((item) => {
                      ...
                      const warehouseDays =
                        override.warehouseDays ?? item.warehouseDays;
                      const transport = isExWorks
                        ? 0
                        : (override.transport ?? item.sellingTransport);
                      const other = override.other ?? item.manualOtherCostAed;
                      return (
                        <tr key={item.productId}>
                          ...
                          <td className="px-4 py-4 text-right">
                            <input
                              ...
                              className="w-28 rounded-lg border px-2.5 py-2 text-right"
                            />
                          </td>
                          <td className="px-4 py-4 text-right">
                          ...
                          <td className="px-4 py-4 text-right">
                            <input
                              ...
                              className="w-32 rounded-lg border px-2.5 py-2 text-right disabled:bg-zinc-100 disabled:text-zinc-400"
                            />
                          </td>
                          <td className="px-4 py-4 text-right">
                            <input
            ...
            {previewResult.success &&
              previewResult.role === "finance_admin" && (
                <div className="grid gap-px border-t bg-zinc-200 sm:grid-cols-4">
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center gap-2 rounded-lg bg-zinc-900 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-60"
          >
            {loading ? (
            ...
            {loading ? "Saving..." : "Calculate & Save Quotation"}
          </button>
        </div>
      </form>
        ...
        <section className="rounded-xl border bg-white shadow-sm">
          <div className="flex items-center justify-between border-b p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold">Quotation Saved</h2>
                <p className="text-sm text-zinc-500">{result.quoteNumber}</p>
              </div>
            </div>
            <p className="text-2xl font-semibold">{money(result.totalSales)}</p>
          </div>
          <div className="overflow-visible">
            <table className="w-full min-w-[720px] text-sm">
              <thead className="border-b bg-zinc-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right">Unit Selling Price</th>
          ...
          <div className="flex justify-end border-t bg-zinc-50 px-6 py-4">
            <button
              type="button"

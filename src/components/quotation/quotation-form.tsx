    ...
    <div className="space-y-8">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        {(previewResult?.success || previewResult?.success === false) && <section className="rounded-xl border bg-white shadow-sm">
          ...
          {!previewResult.success ? <div className="flex items-start gap-3 p-5 text-sm text-amber-700"><AlertCircle size={18} className="mt-0.5" />{previewResult.error}</div> : previewResult.role !== "finance_admin" ? <div className="p-5 text-sm text-zinc-500">Live internal costing is available to Finance / Admin users.</div> : <div className="min-w-0 overflow-x-auto">
